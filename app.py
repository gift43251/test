...
def _detect_country(t_lower, z_lower):
    ...
    # 如果完全沒有通過第一層的靜態名單過濾，則認定非目標範圍
    if not target:
        return None, None, None

    # 【第二層】：自動地理定位系統 (Geocoding API)
    # 如果第一層有名單，直接先嘗試抓取對應經緯度
    coords = LOCATION_COORDS.get(target, None)
    if coords:
        lat, lon = coords[0], coords[1]
    else:
        # 內建無預設座標時，啟動第二層線上自動定位
        try:
            geocoder = Nominatim(user_agent="news_center_monitor_2026")
            location_data = geocoder.geocode(target, timeout=4)
            if location_data:
    ...
    # 成功回傳有效資訊
    if final_country_name and lat is not None and lon is not None:
        return final_country_name, lat, lon
        
    # 【第三層】：不符合或無法定位時，回傳 None（使地圖排除此標記）
    return None, None, None
...
for idx, item_name in enumerate(base_menu):
    if cols_row1[idx].button(item_name, use_container_width=True):
        set_view(item_name)
